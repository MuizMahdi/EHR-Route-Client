...
export class ProviderService 
{
   ...
   public checkProviderAddressExistence(): Observable<any> {
      return this.http.get(this.checkProviderAddressExistenceUrl).pipe(
         catchError(error => { return throwError(error) })
      );
   }
   ...
   public registerInstitutionProvider(providerAdditionRequest:ProviderInstitutionAdditionRequest) {
      return this.http.post(this.registerInstitutionProviderUrl, providerAdditionRequest).pipe(     
         catchError(error => { return throwError(error) })
      );
   }
}
